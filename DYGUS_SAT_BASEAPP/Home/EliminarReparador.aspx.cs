...
{
    public partial class EliminarReparador : Telerik.Web.UI.RadAjaxPage
    {
        ...
        protected void listagemreparadoresregistados_ItemCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "Delete")
                {
                    ...
                    LINQ_DB.Parceiro TECNICOELIMINA = new LINQ_DB.Parceiro();

                    var tecnico = from t in DC.Parceiros
        ...
        protected void listagemreparadoresregistados_NeedDataSource(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
        {
            try
            {
                var carregaClientes = from cliente in DC.Parceiros
                                      where (cliente.ID_TIPO_PARCEIRO == 3 || cliente.ID_TIPO_PARCEIRO == 4) && cliente.ACTIVO == true
                                      select new
                                      {
                                          ID = cliente.ID,
