...
{
    public partial class EditarOrdemReparacao : Telerik.Web.UI.RadAjaxPage
    {
            ...
            if (User.Identity.IsAuthenticated == true)
            {
                try
                {
                    ...
                    if (regra == "Tecnico")
                    {
                        ...
                        //divvaloresmanuais.Style.Add("display", "none");
                    }

                    if (regra == "Reparador")
                    {
                        artigos.Visible = false;
                        artigos.Style.Add("display", "none");
                        divvaloresmanuais.Visible = false;
                        divvaloresmanuais.Style.Add("display", "none");
        ...
        protected bool verificarORC()
        {
            ...
            if (Request.QueryString["ID"] != null)
            {
                ...
                var procuraOrc = from or in DC.Orcamentos
                                 where or.ID_OR_ORIGEM.Value == Convert.ToInt32(id) && or.ID_ESTADO <= 1
                                 select or;

                contaorcs = Enumerable.Count(procuraOrc);

                if (contaorcs == 0)
                {
                    var procuraOrcEA = from or in DC.Equipamento_Avariados
                                       join ea in DC.Ordem_Reparacaos on or.ID equals ea.ID_EQUIPAMENTO_AVARIADO.Value
                                       where ea.ID == Convert.ToInt32(id)
                                       select or;

                    contaorcs = Enumerable.Count(procuraOrcEA);

                    if (contaorcs > 0)
                    {

                        foreach (var item in procuraOrcEA)
                        {
                            ideqp = item.ID;

                            var procurao = from d in DC.Orcamentos
                                           where d.ID_EQUIPAMENTO_AVARIADO.Value == ideqp && d.ID_ESTADO <= 1
                                           select d;

                            foreach (var itemo in procurao)
                            {
                                resultado = true;
                                ClientScript.RegisterStartupScript(this.GetType(), "Success", "<script type='text/javascript'>alert('A ordem de reparação que está a editar contém um orçamento associado que ainda não foi aprovado/rejeitado. Será redireccionado para o mesmo após clicar OK!');window.location='GerirORCCliente.aspx?ID=" + itemo.ID + "';</script>'");
                            }
                        }
                    }
                }
                ...
                {
                    foreach (var item in procuraOrc)
                    {
                        resultado = true;
                        ClientScript.RegisterStartupScript(this.GetType(), "Success", "<script type='text/javascript'>alert('A ordem de reparação que está a editar contém um orçamento associado que ainda não foi aprovado/rejeitado. Será redireccionado para o mesmo após clicar OK!');window.location='GerirORCCliente.aspx?ID=" + item.ID + "';</script>'");
                    }
                }
                        ...
                        if (validaEstado())
                        {
                            ...
                            if (ddlestado.SelectedItem.Text == "Reparação Concluída")
                            {
                                try
                                {
                                    ...
                                    foreach (var item in preco)
                                    {
                                        ...
                                        {
                                            listaValoresCustoArtigo.Add(itemcusto.VALOR_CUSTO.Value * Convert.ToInt32(tbqtdArtigo.Value));
                                    ...
                                    DC.SubmitChanges();
                                    SQLLog.registaLogBD(userid, DateTime.Now, "Editar ordem de reparação", "Foi editada a ordem de reparação para REPARAÇÃO CONCLUIDA com o código: " + ACTUALIZAOR.CODIGO.ToString() + ".", true);
                            ...
                            if (ddlestado.SelectedItem.Text == "Aguarda Autorização do Cliente")
                            {
                                try
                                {
                                    ...
                                    ACTUALIZAOR.ID_ESTADO = Convert.ToInt32(ddlestado.SelectedItem.Value);
                                    DC.SubmitChanges();
                                    SQLLog.registaLogBD(userid, DateTime.Now, "Editar ordem de reparação", "Foi editada a ordem de reparação para AGUARDA STOCK com o código: " + ACTUALIZAOR.CODIGO.ToString() + ".", true);
                                    Response.Redirect("ListarOrdemReparacao.aspx", false);
                                }
                                catch (Exception ex)
                                {
                            ...
                            if (ddlestado.SelectedItem.Text == "Cancelada")
                            {
                                try
                                {
                                    ...
                                    ACTUALIZAOR.ID_ESTADO = Convert.ToInt32(ddlestado.SelectedItem.Value);
                                    DC.SubmitChanges();
                                    SQLLog.registaLogBD(userid, DateTime.Now, "Editar ordem de reparação", "Foi editada a ordem de reparação para CANCELADA com o código: " + ACTUALIZAOR.CODIGO.ToString() + ".", true);
                        ...
                        if (validaEstado())
                        {
                            ...
                            if (ddlestado.SelectedItem.Text == "Aguarda Autorização do Cliente")
                            {
                                try
                                {
                                    ...
                                    ACTUALIZAOR.ID_ESTADO = Convert.ToInt32(ddlestado.SelectedItem.Value);
                                    DC.SubmitChanges();
                                    SQLLog.registaLogBD(userid, DateTime.Now, "Editar ordem de reparação", "Foi editada a ordem de reparação para AGUARDA AUTORIZAÇÃO DO CLIENTE (ORÇAMENTO) com o código: " + ACTUALIZAOR.CODIGO.ToString() + ".", true);
                                    Response.Redirect("ListarOrdemReparacao.aspx", false);
                                }
                                catch (Exception ex)
                                {
        ...
        public void EnviarEmail(List<string> email)
        {
            ...
            {
                client.Send(msg);
                SQLLog.registaLogBD(userid, DateTime.Now, "Editar ordem de reparação", "Foi enviado um email para " + email.ToString() + " a informar que a ordem de reparação se encontra concluida.", true);
            }
            ...
            if (validaCamposArtigos())
            {
                ...
                try
                {
                    ...
                    foreach (var item in verificacliente)
                    {
                        if (item.ID_TIPO_PARCEIRO == 1)
                        {
                            ...
                            var codigo = from art in DC.Artigos
                                         ...
                                         where art.CODIGO.ToString().ToUpper() == tbcodartigo.Text.ToUpper()
                                         select new
                                         {
                                             ID = art.ID,
                        ...
                        if (item.ID_TIPO_PARCEIRO == 2 || item.ID_TIPO_PARCEIRO == 4)
                        {
                            ...
                            var codigo = from art in DC.Artigos
                                         join preco in DC.Artigo_Stocks on art.ID equals preco.ID_ARTIGO
                                         where art.CODIGO.ToLower() == tbcodartigo.Text.ToLower()
                                         select new
                                         {
                                             ID = art.ID,
                    ...
                    if (idArtigo > 0)
                    {
                        ...
                        NOVOARTIGO.VALOR_ARTIGO = precofinal * Convert.ToInt32(tbqtdArtigo.Text);
                        ...
                        Session.Clear();
                        SQLLog.registaLogBD(userid, DateTime.Now, "Editar ordem de reparação", "Foi registado o artigo " + NOVOARTIGO.ID_ARTIGO.ToString() + " na ordem de reparação com o código " + NOVOARTIGO.Ordem_Reparacao.CODIGO.ToString() + ".", true);
                    }
                    else
                    {
                        ShowAlertMessage("Não existe stock disponivel para este artigo! O campo de pesquisa será limpo!");
                        tbcodartigo.Text = "";
                        tbdescricaoArtigo.Text = "";
                        tbqtdArtigo.Text = "";
                    }
        ...
        protected void btnRegistarMO_Click(object sender, EventArgs e)
        {
            if (validaMO())
            {
                ...
                try
                {
                    ...
                    listagemMO.Rebind();
                    Session.Clear();
                    SQLLog.registaLogBD(userid, DateTime.Now, "Editar ordem de reparação", "Foi registado a M.O. " + NOVAMO.DESCRICAO.ToString() + " na ordem de reparação com o código " + NOVAMO.Ordem_Reparacao.CODIGO.ToString() + ".", true);
                }
                catch (Exception ex)
                {
                    ErrorLog.WriteError(ex.Message);
            ...
            try
            {
                if (e.CommandName == "Delete")
                {
                    ...
                    foreach (var clienteeliminado in artigo)
                    {
                        ...
                        DC.SubmitChanges();
                        SQLLog.registaLogBD(userid, DateTime.Now, "Editar ordem de reparação", "Foi eliminado o artigo " + clienteeliminado.ID_ARTIGO.ToString() + " na ordem de reparação com o código " + clienteeliminado.ID_ORDEM_REPARACAO.ToString() + ".", true);
                    }
            ...
            try
            {
                if (e.CommandName == "Delete")
                {
                    ...
                    foreach (var clienteeliminado in artigo)
                    {
                        ...
                        DC.SubmitChanges();
                        SQLLog.registaLogBD(userid, DateTime.Now, "Editar ordem de reparação", "Foi eliminado a MO " + clienteeliminado.DESCRICAO.ToString() + " na ordem de reparação com o código " + clienteeliminado.ID_ORDEM_REPARACAO.ToString() + ".", true);
                    }
        ...
        protected void btnOrcamento_Click(object sender, EventArgs e)
        {
            ...
            if (User.Identity.IsAuthenticated == true)
            {
                try
                {
                    ...
                    ActualizaEstaOR(Convert.ToInt32(id));
        ...
        public void ActualizaEstaOR(int idOr)
        {
            try
            {
                ...
                UPDATEESTADO.ID_ESTADO = 4;
                UPDATEESTADO.DATA_ULTIMA_MODIFICACAO = DateTime.Now;
                DC.SubmitChanges();
                SQLLog.registaLogBD(userid, DateTime.Now, "Editar ordem de reparação", "Foi actualizado o estado da ordem de reparação para ATRIBUIDA E EM REPARAÇÃO com o código " + UPDATEESTADO.CODIGO.ToString() + ".", true);
            }
            catch (Exception ex)
            {
                ErrorLog.WriteError(ex.Message);
        ...
        public void EnviarEmailOrcamentoCliente(string email)
        {
            ...
            {
                client.Send(msg);
                SQLLog.registaLogBD(userid, DateTime.Now, "Editar ordem de reparação", "Foi enviado um email para " + email.ToString() + " a informar novo orçamento na sua conta.", true);
            }
        ...
        protected void ddlestado_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
        {
            ...
            if (ddlestado.SelectedItem.Text == "Aguarda Autorização do Cliente (Orçamento)")
            {
